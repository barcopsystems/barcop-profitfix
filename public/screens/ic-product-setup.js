...
S.InventoryProducts = {
  ...
  showHowTo() {
    App.showHelpModal('How Add Products Works', [
      ...
      { h: 'Sold On The Menu (Food and Misc)', p: ['Most Food and Misc products are things you cook or clean with, not sell straight. But some you do sell as-is: a bag of chips, a bottled soda, a canned non-alcoholic drink. For those, tick Include this item in the Menu Builder in the Sold on the Menu section at the bottom of the form. That lists the product in the Menu Builder as a No Prep item, so you can add it to a menu section with its own price. Leave it off for raw ingredients and supplies so they stay out of the menu picker. Beer and wine are always available on the menu, so they carry no tick.'] },
  ...
  renderForm() {
    ...
    const menuFlagBlock = spec.showUnitType
      ? '<label style="display:flex;align-items:center;gap:10px;cursor:pointer;margin-top:6px;">'
        ...
        + '<span style="font-size:13px;color:var(--t1);">Include this item in the Menu Builder</span>'
        + '</label>'
      : '';

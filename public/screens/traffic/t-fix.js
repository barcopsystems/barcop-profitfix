...
S.TrafficFix = {
  ...
  renderPage() {
    ...
    const timelineLink = '<div style="margin:-4px 0 16px;display:flex;gap:8px;flex-wrap:wrap;"><button class="btn btn-ghost btn-sm pf-timeline">See Your Recovery Timeline</button><button class="btn btn-ghost btn-sm pf-playbook">Read the Traffic Playbook</button></div>';
    ...
    this.container.querySelector('.pf-playbook')?.addEventListener('click', () =>
      { if (window.S && S.RecoveryPlaybook) S.RecoveryPlaybook.open('traffic'); });
    this.wireWorkspace();
  },

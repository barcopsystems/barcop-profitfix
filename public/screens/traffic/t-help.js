    ...
    const sections = [
      ...
      { t: 'Traffic Audit', qa: [
        { q: 'How does the Traffic Audit work?',
          a: 'Click Generate This Month\'s Audit, upload your screenshots and data files, and Bar Cop reads every document you submit alongside the weekly data you have entered. It scores seven sections: Google Business Profile, website, reviews, search and SEO, social media, delivery platforms, and email and loyalty. Each section gets a score based on what the submitted data shows. The overall score is a weighted composite. The report identifies your lowest-scoring section, the specific variables pulling the score down, and the biggest-dollar actions to take before your next audit. The PDF is ready to download within two minutes.' },
      ...
      { t: 'Review Tracker', qa: [
        ...
        { q: 'How do I use the Review Reply Log?',
          a: 'Every time you reply to a review (positive or negative, on any platform), log it: date, source, rating, the reply you wrote, and the action you took. Use Print Blank Sheet to take a paper log behind the bar or to the office and capture replies as you write them. The log gives you the bookkeeper-ready record of which reviews you handled, and Bar Cop credits the Recovery Scoreboard when your logged monthly reply count hits the review velocity target.' },
      ...
      { t: 'Social Media', qa: [
        ...
        { q: 'How do I use the Social Post Log?',
          a: 'When you publish a social post, log it: date, platform, content type (Food/Drink, Event/Promo, Behind-Scenes, Staff/Guest, Reel/Video, Story), a caption snippet, and the link. The log counts your monthly posts automatically and replaces the typed IG Posts/Mo number with a real count when at least one post is logged this month. Use Print Blank Sheet to keep a paper log on the back bar that staff can fill in as they post, then you type the week into Bar Cop on Monday.' },
      ...
      { t: 'Email and Loyalty', qa: [
        ...
        { q: 'How do I use the Email Campaign Log?',
          a: 'Every time you send an email campaign, log it: send date, type (Newsletter, Event Promo, New Menu, etc.), subject line, send count, and open percentage if you know it from your platform. The log gives you the bookkeeper-ready answer to "what did you send this month" and the open-rate metric uses the logged campaign averages when present. Use Print Blank Sheet to keep an ongoing record of campaigns sent across the month.' },

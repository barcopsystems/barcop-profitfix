...
S.BarProducts = {
  ...
  render(container, actions) {
    ...
    btn.className='btn btn-primary btn-sm';btn.textContent='Add Product';
    ...
    if(prods.length===0){
      ...
      const rows=prods.map(p=>{
        ...
        return '<tr><td class="val">'+esc(p.name)+'</td><td>'+esc(p.category||'—')+'</td>'
          ...
          +'<td><div class="row-actions"><button class="btn btn-ghost btn-sm bp-edit" data-id="'+p.id+'">Edit</button>'
          +'<button class="btn btn-danger btn-sm bp-del" data-id="'+p.id+'">Delete</button></div></td></tr>';
      }).join('');
  ...
  showForm(id) {
    ...
    const fa=document.getElementById('bp-form');if(!fa)return;
    ...
    ['bp-coz','bp-pour','bp-cost','bp-price'].forEach(fid=>document.getElementById(fid)?.addEventListener('input',()=>this.calcProduct()));
    if(p)this.calcProduct();
    document.getElementById('bp-name')?.focus();
  },

  getOz(){const v=document.getElementById('bp-size')?.value;if(!v||v==='')return 0;if(v==='custom')return parseFloat(document.getElementById('bp-coz')?.value)||0;return parseFloat(v)||0;},

  calcProduct(){
    const oz=this.getOz(),pour=parseFloat(document.getElementById('bp-pour')?.value)||0;
    ...
    const target=App.data.settings.targets?.bar_pour_cost_pct||22;
    const pours=pour>0?oz/pour:null,cpp=pours?cost/pours:null,pct=cpp&&price?cpp/price*100:null;
  ...
  save(){
    ...
    const oz=this.getOz();
    if(!oz){if(err){err.textContent='Select container size.';err.style.display='inline';}return;}
    const pour=parseFloat(document.getElementById('bp-pour')?.value)||0;
    const cost=parseFloat(document.getElementById('bp-cost')?.value)||0;
    const price=parseFloat(document.getElementById('bp-price')?.value)||0;
    const pours=pour>0?oz/pour:null,cpp=pours?cost/pours:null,pct=cpp&&price?cpp/price*100:null;
    const prod={id:this.editId||App.uid(),name,category:document.getElementById('bp-cat')?.value,vendor:document.getElementById('bp-vendor')?.value.trim(),bottle_size_oz:oz,std_pour_oz:pour,cost_per_unit:cost,menu_price:price,pours_per_bottle:pours,cost_per_pour:cpp,pour_cost_pct:pct,created_at:this.editId?undefined:new Date().toISOString()};
    if(!App.data.bar_products)App.data.bar_products=[];
    if(this.editId){const i=App.data.bar_products.findIndex(p=>p.id===this.editId);if(i>-1)App.data.bar_products[i]={...App.data.bar_products[i],...prod};}
    else App.data.bar_products.push(prod);
    App.saveKey('bar_products').then(()=>{this.editId=null;this.renderList();});
  },

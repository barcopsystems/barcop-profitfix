...
S.BarProducts = {
  ...
  renderList() {
    ...
    // Single onclick handler — never stacks
    ...
    document.getElementById('bp-sel-all')?.addEventListener('click', () => {
      ...
      const chkAll = document.getElementById('bp-chk-all');
  ...
  showForm(id) {
    ...
    fa.innerHTML = '<div class="divider"></div><div class="card">'
      ...
      + '<div class="fw"><input class="suf" type="number" id="bp-pour" value="' + (p?.std_pour_oz || '') + '" step="0.25" placeholder="1.5"/><span class="suf">oz</span></div></div>'
      + '<div class="f" style="width:110px;flex-shrink:0;"><label>Unit Cost ' + tt('unit-cost') + '</label>'
      + '<div class="fw"><span class="pre">$</span><input class="pre" type="number" id="bp-cost" value="' + (p?.cost_per_unit || '') + '" step="0.01" placeholder="0.00"/></div></div>'
      + '<div class="f" style="width:110px;flex-shrink:0;"><label>Menu Price ' + tt('menu-price') + '</label>'
      + '<div class="fw"><span class="pre">$</span><input class="pre" type="number" id="bp-price" value="' + (p?.menu_price || '') + '" step="0.25" placeholder="0.00"/></div></div>'
      + '</div>'

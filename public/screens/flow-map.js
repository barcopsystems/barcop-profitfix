...
S.FlowMap = {
  ...
  NODES: [
    ...
    { id: 'cash', label: 'Cash', full: 'Cash Recovery', layer: 'diagnose', go: 'c-dashboard',
      what: 'Diagnoses liquidity: cash trapped on the shelves, the cash conversion cycle, your 13-week survival runway, and how fast you pay vendors. The one section that reads from everywhere, on-hand value from Inventory, labor cost from Labor, sales from This Week, deposits from Events, and bills from Books, to answer whether you make it to next quarter and where the money really goes.' },
    ...
    { id: 'cash-playbook', label: 'Cash Playbook', full: 'Cash Playbook', layer: 'plan', go: 'c-playbook',
      what: 'The strategic read for Cash Recovery, between the Cash Audit and the Cash Fix. Where cash gets trapped (dead stock, over-ordering, lazy capital) and where it times out (tight weeks, paying vendors early), and the screen that frees each one.' },
  ...
  EDGES: [
    ...
    { from: 'inventory', to: 'cash',      desc: 'On-hand value, usage velocity, and pars drive Trapped Cash, dead stock, and the capital efficiency read.' },
    { from: 'labor', to: 'cash',          desc: 'Scheduled labor cost is the largest cash-out line in the 13-week survival forecast.' },
    { from: 'this-week', to: 'cash',      desc: 'Weekly sales set the cash coming in across the forecast weeks, and the saved P&L feeds the Cash Bridge.' },
    { from: 'events', to: 'cash',         desc: 'Confirmed booking deposits and balances flow in as scheduled cash.' },
    { from: 'cash', to: 'cash-playbook',  desc: 'The Cash Audit flags the gaps; the Cash Playbook is the strategic read on trapped capital, purchasing, runway, and vendor terms.' },
    ...
    { from: 'cash-playbook', to: 'fix',        desc: 'Its Go-to buttons open the Cash Fix to close each gap, plus the screens that feed it.' },

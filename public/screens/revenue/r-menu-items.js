...
S.RevenueMenuItems = {
  ...
  showHowTo() {
    App.showHelpModal('How Menu Builder Works', [
      ...
      { h: 'Adding an Item', p: ['Start from the tile for the kind of item you are adding. Dishes and Mixed Drinks get a recipe builder, so add ingredients and the cost computes itself, or skip the recipe and type a flat cost. No Prep items link straight to an Inventory product, and the cost and menu price both auto-fill from that product (the price stays yours to change). A poured product like draft beer or wine by the glass carries a Pour Size; a food or resale item carries a Portion; bottle beer sells whole. Enter units sold so Menu Engineering can weight the item by how often it sells.'] },
      ...
      { h: 'Importing', p: ['Switch the form to Import File to drop a spreadsheet of your whole menu at once. You map the columns, then Bar Cop lists every item in the file with the price it will land at and, where a price is moving, the price it is moving from. Nothing is saved until you press the button on that screen, which matters most here: a menu import moves prices, and a price move feeds Menu Engineering, the Pricing Review Log and Recovery. Items come in without recipes; edit any item afterward to build its recipe or link a product. Any new section your file carries joins that tab\'s list, so your own sections are in the dropdown straight away. A section you have hidden stays hidden.'] },
  ...
  wireLanding() {
    ...
    // at all: nothing on this screen destroys anything until the button at the bottom.
    this.container.querySelectorAll('[data-confirm-restore]').forEach(b =>
      b.addEventListener('click', () => {
        if (!this._menuReview) return;
        delete this._menuReview.removed[b.dataset.confirmRestore];
        this.renderLanding();
      }));
  ...
  mountImporter() {
    ...
    CSVMapper.mount(el, {
      ...
      subject: t.noun,
      dropSub: dropSub,
      /* ⛔ THE PREVIOUS RUN'S RESULT GOES TOO. `#mi-imp-result` lives INSIDE the upload panel
         below the drop zone, so a second file dropped straight after an import had its columns
         matched underneath the last run's sentence. */
      onState: state => {
        const map = (state === 'map');
        ['mi-imp-cancel-row', 'mi-imp-result'].forEach(id => {
          const el = document.getElementById(id);
  ...
  /* The rows the operator took out. Built through the SAME plan and the SAME row mapper as the rest,
     ...
     ⛔ AND POINTED AT A CLONE, exactly like the live plan. `_planMenuImport` MUTATES its target, so
     handing it the live menu here would apply a removed row's price move to the real item — the one
     mistake this whole door was restructured to make impossible. Same call, same target shape. */
  _menuReviewRemoved() {
    const r = this._menuReview;
    if (!r) return [];
  ...
  menuReviewHTML() {
    ...
    return ImportConfirm.panel({
      ...
      removedRows: this._menuReviewRemoved(),

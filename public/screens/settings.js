...
S.HubSettings = {
  ...
  async loadSample() {
    ...
    // ── 3-month success arc ───────────────────────────────────────────────────
    // The demo tells a focused first-quarter story: a struggling bar adopts Bar Cop
    // and climbs from failing toward target in ~3 months (front-loaded recovery,
    // matching the Fix System's own 90-day example). recalScale lifts each audit's
    // overall + section scores onto that success curve, scaling sections by the same
    // factor so they stay consistent with the (unchanged) metrics, and recomputes
    // the rolling period label so dates always read correctly. Honesty note:
    // historical seed audits are sample narrative; the LIVE engine on real data
    // still produces honest scores for any operation.
    ...
    const recalScale = (arr, rows) => {
      (rows || []).forEach((row, i) => {
        const rec = arr && arr[i];
        if (!rec) return;
        const f = row.o / (rec.overall_score || 1);
        rec.overall_score = row.o;
        if (rec.raw) rec.raw.OVERALL_SCORE = row.o;
        Object.keys(rec.sections || {}).forEach((label, si) => {
          const ns = Math.max(5, Math.min(95, Math.round(rec.sections[label] * f)));
          rec.sections[label] = ns;
          if (rec.raw) rec.raw['S' + (si + 1) + '_SCORE'] = ns;
        });
        rec.audit_period = periodLabel(row.d);
        if (rec.raw) rec.raw.AUDIT_PERIOD = rec.audit_period;
      });
    };
    ...
    // Recalibrate the three audit histories onto the 3-month success arc: a bad-but-
    // normal start, a fast climb, and near/at target by month 3 (Revenue actually
    // crosses 70). Metrics are unchanged, so the lifted scores stay consistent.
    recalScale(App.data.audits,         [{ d:74, o:36 }, { d:42, o:52 }, { d:8, o:68 }]);

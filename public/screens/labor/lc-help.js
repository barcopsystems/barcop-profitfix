...
S.LaborHelp = {
  render(container, actions) {
    ...
    const sections = [
      ...
      { t: 'Actuals: Log Hours, Daily Snapshot, Weekly Summary', qa: [
      ...
      { t: 'Tips', qa: [
        ...
        { q: 'How does the Pool Calculator work?',
          a: 'Two paths. Inside the Shift Close Wizard, the pool runs inline as Step 4 with that shift\'s logged tips and staff hours pre-loaded. Outside of close, the standalone Pool Calculator on the Tips menu lets you split any pool for any date. Pick a date and Bar Cop auto-loads the matching tip log entries. Pick staff and hours auto-fill from logged hours. Two distribution methods: split by hours worked (most common) or equal split. Saving stores the split linked to that shift, which is what the Books Form 8027 Worksheet reads when allocating per-employee tips to the IRS form.' },
        { q: 'What does Tip History show?',
          a: 'Every saved tip pool distribution and every tip log entry, with a By-Shift section, a By-Staff section, and a By-Week section. The By-Shift view groups by shift so you can see exactly how the night\'s pool was split. Use it as the paper trail for payroll and for any tip dispute. If a staff member questions a payout from three weeks ago, the history shows exactly how the split was calculated and from what pool.' }
      ]},
      { t: 'Pay Periods', qa: [
        { q: 'What is the Pay Periods screen?',
          a: 'A view of the last 12 weeks (Monday to Sunday by default), one row per week with status (Open or Closed), total hours, OT hours, and gross wages. Each row has View Detail, Payroll Export, and Close & Lock (or Reopen if already closed). Pay Periods is the bridge between your live operational labor data and what gets handed to payroll. Run it weekly: review the detail, run Payroll Export, close the period.' },
        ...
        { q: 'What does Payroll Export give me?',
          a: 'Payroll Export, under the Payroll menu, builds the file you hand to payroll for a chosen week. Two formats: a formatted workbook (.xlsx) laid out like Month-End Books, and a clean import CSV with columns only for a payroll provider that wants a plain upload. Either one carries one row per staff member: name, position, week dates, regular hours, OT hours, total hours, wage, regular cost, OT premium, and gross, with a total row at the bottom. For tipped positions, it adds tip-credit applied, tips earned, tipped wage paid, and effective hourly rate. The filename carries your bar name and the week so files do not collide. The numbers come straight from Pay Periods, so the export always ties to what you closed.' },

...
S.LaborHelp = {
  ...
  SECTIONS: [
    ...
    { t: 'Roster & Wages', qa: [
      { q: 'What is the difference between Positions and the Staff Roster?',
      ...
      { q: 'Can someone work two roles at different pay rates?',
        a: 'Yes. On the Staff Roster, give the person a Secondary Role and the wage for it; the wage auto-fills from that position\'s default and you can override it. When you log their hours, a Role picker appears so you log each shift under the role they actually worked, and Bar Cop costs it at that role\'s rate, like a server who picks up bar shifts at the bar rate. Those hours also land in the right department, so a bartender\'s prep-cook hours count as kitchen labor, not bar, all the way through to the Books labor split. If they cross 40 hours across both roles in a week, overtime prices off their average rate for the week. Their tipped status still follows their primary role.' }
    ]},
    { t: 'Certs & Coaching', qa: [
      { q: 'How does the certification expiry alert work?',
        a: 'Every certification you track (liquor server permit, ServSafe, food handler, and the rest) carries an expiration date. Bar Cop flags any cert expiring within 30 days as Expiring and any already past its date as Expired, and surfaces both on the Staff Roster with the person and the cert type. A bartender working an expired permit is a regulatory exposure your shift manager should not be guessing at, and the 30-day window gives the person time to renew. If your state requires a cert that is not on the list, pick Other and name it in the notes; it tracks the same way.' },
      ...
      { q: 'Who can see coaching log entries?',
        a: 'The coaching log lives on each person\'s page, so anyone you give access to the Staff Roster screen can see it, along with that person\'s profile, certifications, and training records. Access follows the permissions you set under Team Members: Admin has full access, and Staff sees it only if you grant them the Labor area. Access is granted by operating area, not screen by screen, and there is no read-only role. Treat the log like any personnel file and keep Staff Roster access limited to the people who should be reading HR notes. Retain entries as long as your jurisdiction requires for HR records.' },
      ...
      { q: 'How does training tracking work?',
        a: 'Training is for proving onboarding got done, not for teaching it: there are no videos or quizzes, just a checklist you sign off and date. Build a reusable template on the Training screen (Bartender Onboarding, Server Onboarding, a new POS rollout), set a position if it is role specific, and add the steps in order. Then open a person on the Staff Roster, use Assign Training in their Training section, and check the steps off as they finish. When every step is done the record stamps its completion date, and you pick who signed off so there is a name on it. The Training screen also shows a team status list of who is Not Started, In Progress, or Complete, and each template prints a blank sheet for a trainer to mark up by hand. It is a record-keeping aid, not legal or HR advice.' }
    ]},
    { t: 'Tips & Tip Credit', qa: [
      { q: 'How does tip-out work in Bar Cop?',
        a: 'Tip-out is set per role on Positions, because a server and a bartender tip out different amounts. A role that rings sales and tips out gets a Tip Out percent of sales; a role that only receives, like a busser or barback, stays at 0. In Tip Tracking, Bar Cop figures each earner\'s tip-out off their sales, and you record what each support person actually received, since the real distribution is yours to make. A Collected versus Distributed line flags any gap. Bar Cop calculates the amounts and each person\'s net take-home; how a tip-out is physically paid out is your call and your payroll provider\'s.' },
    ...
    { t: 'Payroll', qa: [
      ...
      { q: 'How does Bar Cop handle overtime?',
        a: 'Hours over 40 in a week are treated as overtime and paid at time and a half. You see it in three places: a live Overtime Watch note while you build the schedule, listing anyone at or near 40 so you can catch it before you post; the Overtime Watch screen, which reads your posted schedules; and the Pay Periods gross, where the time-and-a-half is applied. Salaried staff are exempt and never accrue overtime, so a salaried but non-exempt employee should be set to Hourly instead. Bar Cop uses a 40-hour weekly threshold; some states also have daily overtime rules, so verify the rules for your jurisdiction with your payroll provider.' }
    ]},
    { t: 'Connections', qa: [
      { q: 'What flows from Labor Control to Revenue Recovery?',
        a: 'Three connections, all read-only on the Revenue side, all always on. Logged hours feed the labor cost line on your confirmed week. The same hours feed This Week\'s RPLH, which divides revenue by labor hours. And the Staff Roster, Front of House staff specifically, feeds the Server Check roster. Set the labor data once here and Revenue stays current.' },

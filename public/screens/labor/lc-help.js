  ...
  SECTIONS: [
    ...
    { t: 'Certs & Coaching', qa: [
      ...
      { q: 'Who can see coaching log entries?',
        a: 'The coaching log lives on each person\'s page, so anyone you give access to the Staff Roster screen can see it, along with that person\'s profile, certifications, and training records. Access follows the permissions you set under Team Members. You are the owner, so you see everything. Everyone you invite, Admin or Staff alike, gets an area grid you set: each operating area is either Full Access or No Access, so a person only reaches the coaching log if you grant them Labor. The difference between the two roles is that an Admin can also invite and manage team members. Access is granted by operating area, not screen by screen, and there is no read-only level. Treat the log like any personnel file and keep Staff Roster access limited to the people who should be reading HR notes. Retain entries as long as your jurisdiction requires for HR records.' },
    ...
    { t: 'Tips & Tip Credit', qa: [
      { q: 'How does tip-out work in Bar Cop?',
        a: 'Tip-out is set per role on Positions, because a server and a bartender tip out different amounts. Set Pays Tip Out to Yes for a role that rings sales and tips out, then set Tip Out On to % of Sales, usually 1 to 2 percent, or % of Tips, usually 10 to 20 percent, and enter the number. A role that only receives, like a busser or barback, is left with Pays Tip Out set to No. In Tip Tracking, Bar Cop figures each earner\'s tip-out on whichever basis you set, and you record what each support person actually received, since the real distribution is yours to make. A Collected versus Distributed line flags any gap. Bar Cop calculates the amounts and each person\'s net take-home; how a tip-out is physically paid out is your call and your payroll provider\'s.' },

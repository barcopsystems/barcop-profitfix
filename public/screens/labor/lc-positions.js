...
S.LaborPositions = {
  ...
  showHowTo() {
    App.showHelpModal('How Positions Work', [
      ...
      { h: 'Department and Default Pay', p: ['Each position belongs to a department (Bar, Front of House, Kitchen, Management, or Other) and carries a default pay setup: a Pay Type of Hourly with a default wage, or Salary with a default annual figure (the way you would set a manager). Edit, beside the Department label, is where you add a department of your own if those five do not fit your house. When you add a staff member in the role, their pay type and figure pre-fill from here, so you set it once instead of on every hire, and you can still override it per person.'] },
      { h: 'Tipped Roles and Tip-Out', p: ['Mark a position Tipped if the role earns tip income. Then choose whether it Pays Tip Out: Yes for a role that rings sales and tips out (servers, bartenders), which opens Tip Out On and a Tip Out % field; No for a role that only receives tip-out (bussers, barbacks, runners). Tip Out On is the basis the percent runs on: % of Sales, usually 1 to 2 percent, or % of Tips, usually 10 to 20 percent. Different roles can tip out different percents on different bases. Bar Cop uses this in Tip Tracking: a role set to % of Sales enters its sales and tips out that percent of them, a role set to % of Tips tips out that percent of the cash and card it already entered, a role that only receives gets an editable Received amount, and a role that both pays and receives (a bartender taking the bar share from servers) gets both. It all feeds the Pay Periods tip-credit check.'] },

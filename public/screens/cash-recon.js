...
S.CashRecon = {
  ...
  renderMain() {
    ...
    if (!allV.length && !allD.length) {
      App.setupCard(this.container, {
        ...
        lead: 'Your drawer counts and cash drops flow here from The Floor. Once a few shifts are reconciled, this shows which cashier and which register keep coming up short. Set up a register and run a shift to start the picture.',
        steps: [
  ...
  showHowTo() {
    App.showHelpModal('How Over and Short Works', [
      { p: ['This finds who and what is repeatedly coming up short. Every drawer count you reconcile in The Floor flows here automatically and gets grouped by cashier and by register, so a pattern stands out even when any single night looks minor. No double entry.'] },
      ...
      { h: 'How This Differs From Cash History', p: ['Cash History in The Floor is the log, every count in the order it happened. This is the diagnosis on top of it: who and what to look at. A cash short here also raises a Needs Attention row on the Hub.'] },

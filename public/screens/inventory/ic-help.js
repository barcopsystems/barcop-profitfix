  ...
  render(container, actions) {
    ...
    const sections = [
      { t: 'Getting Started', qa: [
        ...
        { q: 'Where do I start?',
          a: 'Three steps to get Inventory Control producing real data. First, add your storage locations on the Set Locations screen so each one is ready to pick when you add products. Second, add your vendors on the List Vendors screen with an email address so Bar Cop can email orders straight to them. Third, add every product you stock on the Add Products screen with its container size, pour size, unit cost, and menu price; the first location you give a product becomes its ordering home. Once products are in, open each location and drag the products into the order you walk the shelf so counts follow the room, then take your first count and receive your first delivery and the reports start working. The Getting Started checklist on the Hub sidebar walks you through the same sequence with a checkbox per task.' },
      ...
      { t: 'Products, Batches, Locations, and Vendors', qa: [
        ...
        { q: 'How do Locations work?',
          a: 'Locations are the physical storage spots you count: main bar, back bar, walk-in cooler, dry goods storage, prep kitchen, beer cooler. Add one on the Set Locations screen by naming it and checking off the products kept there. Category tabs filter the checklist so a bottle-beer cooler gets built without scrolling past every spirit, and your checks carry across the tabs with a running count of what you picked. A product can live in more than one location, and the first place you put it becomes its ordering home. Open a location to add or pull products and drag them into the order they sit on the shelf, so Take Inventory walks the room the way you do instead of alphabetical. Reorder the locations themselves on the main list to set which one you count first. Delete moves a location to a Deleted section at the bottom; click Restore to bring it back, and product assignments are kept either way.' },
        { q: 'How do Vendors work?',
          a: 'Add a vendor on the List Vendors screen with its rep, phone, email, delivery days, payment terms, and account number. Email matters most, because Bar Cop uses it to send orders straight from Order History. Open a vendor to edit it and you also see every product you buy from them and their most recent cost changes in one place. Rename a vendor and every product pointing at the old name follows automatically. When you Receive Delivery, you pick the vendor and the system pre-fills last-known prices on the products you receive from them. The Order Sheet groups suggested reorder items by vendor so you can place orders one supplier at a time, and that same price history feeds Profit Recovery\'s Vendor Watch.' },
        { q: 'What are Prep Batches?',
          a: 'Prep batches are the things you make in-house from other products: a frozen margarita mix, simple syrup, a marinara base, a demi-glace. Build the recipe once on the Prep Batches screen and any menu item can use the batch as an ingredient. Set the Batch Yield (how much the whole batch makes) and the Serving Size (how much one drink or plate pulls from it), and Bar Cop divides the two for servings per batch, then splits the batch cost across them for a cost per serving. The batch category sets which products you can add: a Cocktail Mix or Syrup pulls bar ingredients, a Sauce or Stock pulls kitchen ingredients. Line costs use each ingredient\'s current cost, so when a price changes the batch cost updates on its own, and that cost per serving flows straight into any recipe that uses the batch.' }
      ]},
      { t: 'Taking Inventory and the Bottle Slider', qa: [
        { q: 'How do counts work?',
          a: 'Open Take Inventory and pick the locations you want to count: one location for a partial count, several or all for a full count. Bar Cop walks you through each location in turn, with the products inside that location appearing in the order you arranged on the location\'s edit page. For every product you record full unopened units plus how full any open bottle is. Submitting saves a dated snapshot to Count History. Usage, variance, and movers are all measured between two counts, so the first count is your starting point and every report needs a second count to compare against.' },
      ...
      { t: 'Operations', qa: [
        { q: 'What is the Operations section for?',
          a: 'Three logs that capture inventory events outside of normal counts, deliveries, and orders: Transfer Log, Adjustment Log, and Empties Log. They give you an accountability record for every bottle that moves between locations, every documented loss or found-stock entry, and every container that leaves the building. None of them change product totals on their own (a transfer just moves location, an adjustment is a documented attribution), but they feed Theft Risk, Books, and the audit trail your bookkeeper expects.' },
        { q: 'What is the Transfer Log?',
          a: 'Every time inventory moves between two of your locations during a shift (stockroom to front bar, walk-in to kitchen line, prep station to bar well), log it here. Fields: date and time, product, quantity and unit, From location, To location, performed by, optional witnessed by, notes. Filters let you pull transfers by date range, by product, or by either side of the move. Saving does not change product totals because nothing left the building, only its location changed. The log is the accountability record. Each log also has a Worksheet button that gives you a printable PDF sheet to walk with during the shift, then enter after close.' },
        { q: 'What is the Adjustment Log?',
          a: 'When stock leaves or comes back into inventory outside of normal sale, log it here. Reason codes: Damage, Theft, Expiration, Found, Other. Each entry captures the product, quantity and unit, direction (Loss out or Found in), unit cost at the time of the adjustment, performed by, optional witnessed by, and notes. The system pre-fills direction based on reason (Found is in, everything else is out). Filters let you pull adjustments by date, product, or reason. Confirmed Theft entries from the last 90 days feed the Theft Risk score in Profit Recovery directly, so the more honest the log, the more accurate the scorecard.' },
        ...
        { q: 'What is the Empties Log?',
          a: 'Tracks empty container disposition for two reasons: legal compliance in jurisdictions that require it, and deposit redemption tracking where bottle deposits apply. Fields: date, product, quantity and unit, deposit per unit, disposition (Recycle, Return for Deposit, Trash), performed by, notes. The list shows a Deposit Value per entry so you can tally what is owed back across the filtered range. The Worksheet button gives you a printable take-along sheet to fill during the shift.' }
      ]},
      { t: 'Reports', qa: [
        { q: 'What is Dynamic Pars?',
          a: 'Par recommendations built from your real usage, not gut feel. The math: average weekly usage over the last N weeks times your delivery cycle in weeks, plus a safety buffer. Defaults: 8-week window, 7-day cycle, 30% buffer. Adjust those in the Settings card at the top of the screen and the per-product suggestions recalculate live. The table flags every product as Increase, Reduce, or No Change and sorts the Increase and Reduce rows first so changes are easy to see. Bulk-select the rows you want to apply, click Apply Selected, and Bar Cop updates par_level on each product in one save. For bottle beer with case size, suggestions round up to whole cases. Run this monthly or quarterly to keep par levels honest as your menu mix and traffic patterns shift.' },
        { q: 'How is the Usage Report calculated?',
          a: 'Usage equals starting stock plus purchases minus ending stock, measured between two counts. Starting and ending stock come from the counts. Purchases come from any delivery logged between the two count dates. Quantities are in each product\'s stock unit (cases for bottle beer, bottles for liquor and wine, kegs for draft). Usage here is gross consumption; it is not reduced by comps or waste (the Variance Report handles those). It feeds COGS, variance, and the movers analysis.' },
        { q: 'How does the Variance Report work?',
          a: 'Variance compares what you used (from your counts) against what your POS says you sold for the same period. Bar Cop converts what you used into ounces sold using each product\'s pour size, then compares it to the ounces your POS rang up. Known comps and waste are subtracted first, so only unexplained loss shows. Under 5 percent variance is OK, 5 to 15 percent is Watch, over 15 percent is Flag. A high variance points to over-pouring, waste, or theft.' },

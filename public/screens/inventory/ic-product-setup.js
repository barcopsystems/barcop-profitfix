...
S.InventoryProducts = {
  ...
  showHowTo() {
    App.showHelpModal('How Add Products Works', [
      ...
      { h: 'Checking Where Everything Goes', p: ['A distributor order guide is one file with kegs and cases in it, and an export from another program is usually one file with everything in it. So once the columns are matched, every import stops here: Bar Cop lists every row it found and shows you what happens to each one before anything is written. It is the same screen and the same press whether the file sorted itself perfectly or not, and it is your chance to drop anything you no longer carry.', 'Every row sits in one section, and the sections add up to the rows in your file. A section per category holds what is going in, and its count is exactly the rows under it. Not Going In holds everything that is not, with the reasons counted on it. Anything Bar Cop could not work out sits at the top under Not Sorted Yet, already open, and that is the only part you have to do. The Add button is your file less the ones not going in, so there is no arithmetic left for you to do.', 'Every row also carries a What Happens column, so you never have to work out which rows the counts are talking about. A row going in names the category it is going to. A row that is not going in says why: already in your list, already there, hidden on the Inactive tab, repeated in this file, a row with no name, a heading row, or a charge on your invoice.', 'Bar Cop fills in whatever the product name or a column in your file settles on its own. Bourbon, Vodka and Gin are Liquor, and it knows the common spirit and wine brands by name. It leaves a beer style alone on purpose, because an IPA can be a keg or a case and the file does not say which, and it would rather ask than put your product in the wrong place. It also picks out what an order guide carries that you do not stock: section headings, subtotal lines, keg deposits, delivery charges. None of that is thrown away. It sits in Not Going In with its reason on it, and if Bar Cop has one wrong, tick it, pick a category and press Move To and it becomes a product like any other. Every row has a Remove button too, for anything you no longer carry.', 'If the file turns out to be all one category, like a draft list or a wine list, anything Bar Cop cannot read on its own goes to the card you started from, because that is the answer you already gave it. When the file holds several categories it never does that: what it cannot place stays at the top for you.', 'If your file does not name a supplier, Bar Cop asks once and puts that vendor on every product it adds that does not already name one, and adds them to your vendor list so you can order from them. Nothing is written to your account until you press Add, and that button says exactly how many products are coming in.'] },
  ...
  async runImport(rows) {
    ...
    if (ok) {
      ...
      this._formCategory = null;
      this._import = null;
      /* ⛔⛔ ONE SENTENCE. IT ENUMERATED EVERY BUCKET AND IT LOOKED LIKE A CRASH (Kyle, 2026-08-15,
         on a real broadline food guide): *"this output at the top of the page looks really bad..
         almost looks like broken code.. can't it just say 'Added 21 products.'"* He was reading
         `6 heading rows: *** PRODUCE ***, *** MEAT & SEAFOOD ***, *** DAIRY & FROZEN *** ...`,
         which is exactly what a file's own furniture looks like when you print it back at someone.
         ⭐ AND THE REASON IT CAN BE SHORT NOW IS NEW. Every one of those rows was already on the
         REVIEW SCREEN, named, in Not Going In, with its reason in the What Happens column, and the
         operator pressed a button on that screen to get here. `runImport` has exactly one caller
         (the review's Add), so there is no path to this message that skipped it. Repeating the
         whole breakdown is telling them what they just read, in the ugliest available form.
         ⛔ WHAT THIS MUST STILL CATCH: the review PREDICTS what will land and this reports what
         DID. If those disagree the operator has to hear it here, because the review is gone. So
         the buckets are not deleted, they are held for the two cases that carry information the
         review could not: nothing landed at all (below), and fewer landed than were promised. */
      const promised = this._routing ? this._routeReadyCount() : imported.length;
      const short = 'Added ' + imported.length + ' product' + (imported.length === 1 ? '' : 's') + '.';
      this._importMsg = imported.length < promised
        ? short + ' ' + (promised - imported.length) + ' of the ' + promised
          + ' on the review did not make it: ' + buckets().join('. ') + '.'
        : short;
      this.renderLanding();
    } else {

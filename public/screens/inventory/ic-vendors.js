...
/* ── Inventory Control — Vendors (ic_vendors) ─────────────────────────────────
   ...
   a vendor by name. Vendor pricing auto-feeds the Vendor Tracker under Vendors.
...
S.InventoryVendors = {
  ...
  showHowTo() {
    App.showHelpModal('How Vendors Work', [
      ...
      { h: 'Pricing Feeds The Vendor Tracker', p: ['Each time you apply a cost change in Receive Delivery, Bar Cop logs it against the vendor. That same history feeds the Vendor Tracker under Vendors, so a vendor quietly raising prices shows up before it eats your margin.'] }
    ]);
  },
  ...
  renderList() {
    ...
    if (vendors.length === 0) {
      listSection = '<div class="card" style="margin-top:18px;padding:14px 20px;"><div style="font-size:12px;color:var(--t3);line-height:1.6;">No vendors yet. Add one above. '
        + 'Products link to a vendor, and vendor pricing feeds the Vendor Tracker under Vendors.</div></div>';
    } else {
  ...
  wireList() {
    this.container.onclick = ev => {
      ...
      if (vpb && this._vendorReview) {
        delete this._vendorReview.removed[vpb.dataset.confirmRestore];
        this.renderList(); return;
      }
  ...
  _vendorReviewRemoved() {
    const r = this._vendorReview;
    if (!r) return [];
    const gone = r.rows.filter(x => r.removed[x._rid]);
    if (!gone.length) return [];
    return this._buildVendorRows(gone).list.map((x, i) => Object.assign(
      this._vendorReviewRow(x, (gone[i] || {})._rid),
  ...
  vendorReviewHTML() {
    ...
    return ImportConfirm.panel({
      ...
      removedRows: this._vendorReviewRemoved(),

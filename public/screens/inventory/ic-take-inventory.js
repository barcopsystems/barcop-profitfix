...
S.InventoryTakeInventory = {
  ...
  showHowTo() {
    App.showHelpModal('How the Inventory Count Works', [
      ...
      { h: 'Pick Your Locations', p: ['Set the date and who is counting up top, then pick where. Most operators count one location at a time and come back for the rest later. Pick a single location for a quick section count, or pick several to run a full inventory in one session. You count and finalize each location\'s products together. Bar Cop keeps one count per location per day, so counting Back Bar in the morning and the Store Room after close puts both on the same count for that day and leaves every other location on it alone. Pick a location you already counted today and Bar Cop offers to open that count and correct it, landing you on the shelf it names, because two counts of the same shelf in one day cannot be used as a usage pair. Hit Worksheet to print a blank count sheet in your shelf order, so you can pencil in full and open on the floor before you key the numbers in.'] },
  ...
  renderSetup() {
    ...
    if (prods.length === 0) {
      App.setupCard(this.container, {
        ...
        lead: 'A count is the backbone of Inventory. It sets your stock value and feeds usage, variance, and your reorder list. Add your products and you can count.',
        steps: [
    ...
    if (locs.length === 0) {
      ...
      const tiles = locs.map(l => {
        ...
        return '<button type="button" class="ti-loc-tile" data-loc="' + esc(l.name) + '" style="text-align:left;display:flex;align-items:center;gap:11px;padding:13px 14px;border-radius:8px;border:1px solid var(--b2);background:var(--gold-tint);cursor:pointer;transition:background .12s,border-color .12s;">'

  ...
  render(container) {
    const sections = [
      { t: 'About Bar Cop', qa: [
        ...
        { q: 'What does the monthly Audit do that the dashboard does not?',
          a: 'The dashboard shows you the current state. The Audit scores the operation. It is a structured diagnostic on a 0 to 100 scale across every section of every Recovery system, written like a coaching report instead of a raw metric dump. It names each gap, attaches a dollar figure to it, and ranks the action items by annual impact. One Audit per month is included in your subscription. The recurring value of Bar Cop is that scored monthly report plus the Recovery Scoreboard proving the work is paying off.' },

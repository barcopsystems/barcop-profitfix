...
/* ── Shift Control — Variance Log (writes sc_variances) ───────────────────────
   ...
   counted cash. The variance tolerance comes from Hub Settings (Rule 18).
...
S.ShiftVarianceLog = {
  ...
  calc() {
    ...
    const status = this.statusOf(variance);
    varEl.textContent = (variance >= 0 ? '+' : '') + App.fmtCurrency(variance);
    varEl.className = 'calc-val ' + (status === 'Short' ? 'warn' : status === 'Over' ? 'dim' : 'good');
    statEl.textContent = status;
    statEl.className = 'calc-val ' + (status === 'Short' ? 'warn' : status === 'Over' ? 'dim' : 'good');
  },

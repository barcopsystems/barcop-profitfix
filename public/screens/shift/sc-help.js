  ...
  SECTIONS: [
    { t: 'Getting Started', qa: [
      { q: 'What does Shift Control do?',
        a: 'Shift Control captures the operating week: your imported POS sales (revenue and covers), cash drops, the safe log, drawer variances, voids and comps, maintenance issues, walked tabs, floor incidents, your opening and closing checklists, and your pre-shift line-ups. It is one of three Control systems (Inventory, Labor, Shift) that capture daily operations. Your imported sales are the single source for the weekly revenue number every Recovery system reads. Cash data feeds Profit Recovery Cash Reconciliation and the Profit Audit. Voids and comps feed Loss Prevention. Run Shift Control well and the rest of Bar Cop produces honest numbers without you typing the same data twice.' },
      { q: 'Where do I start?',
        a: 'A little setup, then a weekly habit. To set up, build your opening and closing checklists on Build Checklists and add your registers on Add Registers. Then each week, open the Shift dashboard and import this week\'s POS sales: one "sales by day" export, dropped once, lands the whole week. The Getting Started checklist on the Hub lists the Shift Control tasks in order.' },
    ...
    { t: 'Cash Control', qa: [
      ...
      { q: 'What do the variance colors mean?',
        a: 'Within tolerance is green. Short, meaning less cash than expected, is red. Over, meaning more cash than expected, is amber. A drawer that was never counted shows grey as Not Counted. Cash History records every variance and the Bar Cop Audit counts the out-of-tolerance ones, so a bartender consistently short across shifts is a pattern Bar Cop surfaces without you tracking it on paper.' }
    ]},
    { t: 'Operations Logs', qa: [
      { q: 'What goes in the Void and Comp Log?',
        a: 'Every voided ticket and every comped item, logged by the dollar amount (a whole comped table is one line, not one per item). The item is optional, picked from the menu-and-inventory dropdown when you want the link or left off. The Reason carries the classification: customer-facing comps such as service recovery, goodwill, regular or VIP, and promo are loss and feed Loss Prevention, while Staff Meal and Shift Drink are policy expense tracked as cost in Books and the Annual Review. Enter a whole shift at once in the batch builder, or edit a single row to add a check number or a note. The log feeds the loss total on Loss Prevention and the Bar Cop Audit\'s void and comp checks; for the per-server pattern, Sales Integrity reads comps off your POS report.' },
      ...
      { q: 'What is the Incident Log for?',
        a: 'The written record for anything on the floor you would want documented later: an injury or slip, a fight or ejection, a guest cut off or refused service (your dram-shop record), theft or a break-in, property damage, a customer complaint or illness claim, police or EMS called. Capture the date and time, type, severity, who was involved, who witnessed it, what happened, and the action you took. It is a record-keeping aid, not legal advice, and the signed Export PDF is what protects you if an incident ever becomes a claim or a lawsuit. Leave one Open while it still needs follow-up; mark it Resolved when it is closed out.' }
    ]},
    { t: 'Checklists and Briefing', qa: [
      { q: 'How do the checklists work?',
        a: 'The Run Checklists screen runs both routines. Toggle Opening or Closing up top (it starts on the one that fits the time of day), and the items come from a checklist you built on Build Checklists or a built-in default. Tap items to check them off, then Save to record the manager who completed it, the date, and the completion percentage. Completed runs land in the filterable history below, where View shows the per-item detail and Export PDF saves the record, so you can see which shifts ran the open and close properly and which skipped steps.' },

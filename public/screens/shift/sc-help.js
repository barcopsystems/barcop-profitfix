    ...
    const sections = [
      ...
      { t: 'Operations Logs', qa: [
        { q: 'How does the 86 List work?',
          a: 'When something runs out, 86 it: pick the menu item or the inventory product from the dropdown, optionally add a reason (out of product, equipment down, no prep time). The item shows as a large card visible from any screen until someone marks it Back In Stock. Active during-shift use is mobile-first so a bartender on the floor can 86 a product in three taps. The Worksheet button on the 86 List gives you a printable PDF grid the bar can mark up during service, then the manager enters into Bar Cop after close. Repeat 86s on the same product across recent shifts flow into Inventory Control as par-level alerts.' },
        { q: 'What goes in the Void and Comp Log?',
          a: 'Every voided ticket and every comped item, with who authorized it. The Item field suggests menu items from your Revenue Recovery menu (so rollups stay clean) but lets you type a custom name. Comps carry a Category dimension (Customer Comp, Service Recovery, Staff Meal, Shift Drink) so policy expense does not get conflated with loss. The log feeds Profit Recovery Theft Risk Scorecard (volume and pattern matter) and the Profit Audit exception analysis. The Worksheet button on the Void/Comp Log gives you a printable PDF tally sheet to clip to the POS during service; manager enters into Bar Cop after close.' },
        { q: 'What is the Comp Authorization Threshold?',
          a: 'Under Shift Control Setup, Comp Authorization sets the dollar amount above which a comp on the Void/Comp Log should have a manager in the Authorized By field (default $25). Saving a comp over the threshold without a manager pops a soft warning the operator can override; every override is flagged in Theft Risk under Unauthorized Large Comps. Set the threshold to 0 to disable the warning entirely if you do not want it.' },

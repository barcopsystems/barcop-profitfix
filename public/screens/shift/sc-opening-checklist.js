...
S.ShiftOpeningChecklist = {
  ...
  renderMain() {
    ...
    const runner = '<div class="card"><div class="card-title">Opening Checklist</div>'
      ...
      + '<input type="text" id="oc-by" value="' + esc(this._run.completed_by) + '" placeholder="Manager on duty"/></div>'
      + '</div>'
  ...
  _bindRunner() {
    const sync = () => {
      ...
      this._run.completed_by = document.getElementById('oc-by')?.value || '';
  ...
  renderDetail(id) {
    ...
    this.container.innerHTML = '<div class="screen">'
      + '<div style="margin-bottom:14px;"><button class="btn btn-ghost btn-sm" id="oc-back">&#8592; Back to Opening Checklist</button></div>'
      ...
      + '</div></div>';
    this.container.onclick = ev => { if (ev.target.closest('#oc-back')) this.renderMain(); };
  },

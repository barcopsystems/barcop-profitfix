...
S.ShiftWalkedTabs = {
  ...
  formFields(r) {
    ...
    return '<div class="form-row data-row" style="gap:12px;">'
      ...
      + '<div class="f" style="flex:0.9;min-width:0;"><label>Time</label><input type="time" id="wt-time" value="' + esc(r?.time || this.nowTime()) + '"/></div>'
      + '<div class="f" style="flex:1.5;min-width:0;"><label>Reason</label><select id="wt-reason">' + reasonOpts + '</select></div>'
      ...
      + '<div class="f" style="flex:0.65;min-width:0;"><label>Amount</label><div class="fw"><span class="pre">$</span><input class="pre" type="number" id="wt-amount" min="0" step="0.01" value="' + v(r?.amount) + '" placeholder="0.00"/></div></div>'
      + '<div class="f" style="flex:1.5;min-width:0;"><label>Manager</label><select id="wt-mgr">' + App.staffOptions(r?.manager_id || App.activeManagerId(), { placeholder: 'Select staff...' }) + '</select></div>'
      + '</div>'

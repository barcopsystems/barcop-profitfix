...
/* ── THE SHIFT IMPORT LANE ── sales (file or hand-entry) and the cash report.

   Moved out of `sc-dashboard.js` on 2026-08-11, BY BYTE RANGE, so every member below is the code
   that shipped rather than a retyping of it. Same reason as the Labor lane a few hours earlier:
   Close The Week hosts these doors and that cockpit is being deleted, and while the doors lived
   inside the page, deleting the page deleted half the import lanes on the screen that replaces it.

   ⛔⛔⛔ NO "DEFAULT" HOST. Every accessor here used to read `host ? host.X() : this.X()`, where
   the right-hand side meant "the Shift cockpit". On this object there IS no cockpit — no `render`,
   no `container`, no `weekStart` — so the fallback could not come with the code. Both pages pass a
   host now: `SC_HOST` on the cockpit, `SC_HOST` on Close The Week. Nothing here reaches a page.

   A HOST IS: { zone: {import, cash}, takeover, container(), rerender(), weekStart()?, weekEnd()?,
                advance(step)? }
   `weekStart`/`weekEnd` are OPTIONAL and matter to ONE door: the Enter Manually grid builds seven
   day rows and pre-fills them, so it has to build the seven days the operator can see. A file
   carries its own dates and never asks. `advance` is the cockpit accordion and Close The Week has
   no steps, so it offers none.

   ⚠ THE WIRING IS SPLIT, AND THAT WAS THE HARD PART. `sc-dashboard.wire()` was ONE
   `container.onclick` holding eleven lane handlers interleaved with seven pieces of cockpit
   chrome, and Close The Week ran the WHOLE thing just to get the confirm screen’s buttons. The
   lane’s half is `_ckHandle(ev)` below — it returns true when it has handled the event — and each
   host calls it from its own handler. Labor never had this problem: its confirm wiring was
   already a member of its own.  */

S.ShiftLane = {

  /* The landing message, the dates it is about, and which zone it belongs to. Read by whichever
     page is hosting. DECLARED here rather than appearing at first assignment, because a field that
     exists only at runtime is invisible to every member-based instrument in the suite. */
  _flash: null,
  _flashDates: null,
  _flashZone: null,
  /* The two confirm screens and their one-press guards, declared for the same reason. */
  _salesReview: null,
  _cashReview: null,
  _salesReviewWriting: false,
  _cashReviewWriting: false,
  _pendingCashRows: null,
  _importingSales: false,
  _importingCash: false,
  _applyingCashMap: false,

  /* ⚠ THE LANE’S OWN STORE ACCESSOR. `shifts()` is 61 bytes reading `App.shiftData.sc_shifts`, and
     three members here need it (the review summary, the manual grid and the save). The cockpit keeps
     its copy until it is deleted; `verify-lane-extractable` pins that the two agree while both
     exist ([[the-loop]] #54: when a quantity is computed twice, test the EQUALITY). */
  ...
  /* ⚠ `CK_ZONE` IS NOT HERE ANY MORE, and it very nearly was. It held `sc-ck-import` /
     `sc-ck-cash`, which are the SHIFT COCKPIT'S element ids, and its only reader was the fallback
     inside `_ckZone` — so the moment that fallback went it was a table nothing read
     ([[the-loop]] #25). It moved to that page's own `SC_HOST`, where ids belong.
     ⛔ I HAD ALREADY DONE EXACTLY THIS FOR `LC_ZONE` ON THE LABOR LANE AND STILL LEFT THIS ONE
     BEHIND — "the same as the one I just did" is where the miss lives ([[lessons-paid-for]] #12),
     and it took a harness going red to find it. `CK_TITLE` stays, because BOTH hosts read it. */
  ...
  /* ⛔⛔⛔ AND THERE IS NO `null` HOST ANY MORE. Every accessor below used to read
     `host ? host.X() : this.X()`, where the right-hand side meant "the Shift cockpit". On this
     object there IS no cockpit: `render`, `container`, `actions`, `weekStart` and `weekEnd` are
     members of a PAGE and this is not one. Both pages pass a host now. NO HOST MEANS NOTHING, never
     "guess": `_ckZone` answers '' and every caller already refuses on an empty zone, `_ckContainer`
     answers null and its callers already guard. A silent fallback to some other page's ids is the
     one behaviour this whole contract exists to prevent. */
  _ckHost: null,
  /* Which half of the sales toggle is showing. It lived on the COCKPIT as a page field, and it is
     lane state: the grid, the toggle and the save all read it, and Close The Week writes it when the
     operator flips the toggle. One lane, one object, one state. */
  _salesMode: null,
  ckClaim(host)  { this._ckHost = host || null; },
  _ckZone(key)   { return ((this._ckHost && this._ckHost.zone) || {})[key] || ''; },
  ...
  /* ⚠ STILL OPTIONAL, AND THE FALLBACK IS NOW `App` RATHER THAN A PAGE. A host that names no week
     gets the app's own current week, not some cockpit's selector — the drop zones never ask, and a
     grid drawn for a week nobody chose is better than one drawn for a hidden page's week. */
  _ckWeekEnd()   { return (this._ckHost && this._ckHost.weekEnd)   ? this._ckHost.weekEnd()   : App.nextSunday(); },
  _ckWeekStart() { return (this._ckHost && this._ckHost.weekStart) ? this._ckHost.weekStart() : App.weekStartFor(this._ckWeekEnd()); },
  ...
  _ckContainer() { return this._ckHost ? this._ckHost.container() : null; },
  _ckRerender()  { if (this._ckHost) this._ckHost.rerender(); },
  /* ⭐ THE STEP THE HOST SHOULD OPEN NEXT, optional for the same reason `weekStart` is: a host names
     what it has. This replaces six `this._openStep = '...'` writes — the lane reaching straight into
     a PAGE's accordion, invisible to Close The Week and dead the day the cockpit goes. */
  _ckAdvance(step) { if (this._ckHost && this._ckHost.advance) this._ckHost.advance(step); },
  /* ⛔⛔ AND THE MANUAL TICK GOES THROUGH THE HOST TOO, WHICH IS A BEHAVIOUR CHANGE WORTH STATING.
     `_doImportSales` called `this.setDone('import', true)` under the comment *"a cockpit import is a
     deliberate 'the week is in' action"*. That was true while only the cockpit could run it. It runs
     under BOTH hosts now, and `_doneKey()` is `'sc_cockpit_done_' + this.weekEnd()` — the COCKPIT'S
     week — so an operator importing Aug 3-9 on Close The Week ticked a box filed under whatever week
     the hidden cockpit happened to be on, and that tick reaches the Hub briefing through
     `hubSteps()`. The cockpit still marks its own step; a hub page that says it has no Mark Done
     anywhere now genuinely has none. Surfaced to Kyle 2026-08-11 and not yet ruled on: if he wants
     the tick from both pages, `markIn` goes on Close The Week's host and nothing else changes. */
  _ckMarkIn() { if (this._ckHost && this._ckHost.markIn) this._ckHost.markIn(); },
  ...
  async _doImportSales(rows, opts) {
    ...
    if (!allToAdd.length) {
      ...
      if (opts.cleared) {
        ...
        this._ckAdvance('cash'); this._ckRerender(); return;
      }
    ...
    this._ckMarkIn();   // the HOST decides what an import means for its own page — see `_ckMarkIn`
  ...
  /* ── THE LANE’S OWN CLICK HANDLING ───────────────────────────────────────
     ⛔⛔⛔ THIS WAS THE HARD PART OF THE MOVE. These eleven handlers lived inside the Shift
     cockpit’s `wire()`, interleaved with seven pieces of that page’s own chrome (the step heads,
     Mark Done, the week stepper), in ONE `container.onclick`. Close The Week ran the WHOLE thing
     just to get the confirm screen’s buttons — which worked only because none of the chrome
     selectors is on screen while a confirm screen is up. Measured, not assumed; and it is exactly
     the kind of arrangement that is fine until the day it is not.
     ⚠ RETURNS TRUE WHEN IT HANDLED THE EVENT, so each host calls it FIRST and falls through to
     its own controls otherwise. Not a second `container.onclick`: that is an assignment, so
     whichever page wired last would silently win and the other page’s controls would be dead.  */
  _ckHandle(ev) {
    const sm = ev.target.closest('[data-salesmode]');
    /* ⚠ THIS LINE ARRIVED WITH THE `wire()` SPLIT, AFTER the page-state sweep had already run over
       this file, so it still held `this._openStep` and `this.render(this.container, this.actions)`
       — three members the lane does not have and a call that would have THROWN the moment an
       operator pressed Enter Manually. Caught by `verify-method-refs`, which resolves every
       `this.X()` in the tree; the dangling-reference check I ran by hand was clean because I ran it
       BEFORE the split. Re-run a structural check after every structural change, not once. */
    if (sm) { this._salesMode = sm.dataset.salesmode; this._ckAdvance('import'); this._ckRerender(); return true; }
    if (ev.target.closest('[data-savesales]')) { this.saveManualSales(); return true; }
    ...
    if (sconf && this._salesReview) {
      ...
      this._ckRerender(); return true;
    }
    if (ev.target.closest('[data-salesreview-go]')) { this._runSalesReview(); return true; }
    ...
    if (this._cashReview) {
      ...
      if (rm) { this._cashReview.removed[rm.dataset.confirmRemove] = true; this._ckRerender(); return true; }
      ...
      if (rs) { delete this._cashReview.removed[rs.dataset.confirmRestore]; this._ckRerender(); return true; }
    ...
    if (ev.target.closest('[data-cashreview-go]')) { this._runCashReview(); return true; }
    if (ev.target.closest('[data-cashreview-back]')) {
      ...
      this._clearTakeover(); this._ckRerender(); return true;
    }
    if (ev.target.closest('[data-salesreview-back]')) {
      ...
      // ⚠ AND THE PAGE COMES BACK WITH IT. The carried mapper is spent once its rows have gone to
      // the confirm screen, so leaving the takeover set would re-attach a dead node over the step.
      this._salesReview = null; this._clearTakeover(); this._ckRerender(); return true;
    }
    return false;
  },
  /* A host with no click handling of its own can hand its container straight here. Close The Week
     uses this; the cockpit calls `_ckHandle` from inside its own handler because it has chrome to
     fall through to. */
  ckWire(c) { if (c) c.onclick = ev => this._ckHandle(ev); },
};

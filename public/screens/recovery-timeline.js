...
S.RecoveryTimeline = {
  ...
  render(container, moduleKey) {
    ...
    if (!start) {
      ...
      const dN = this.daysBetween(start, today);
      const next = phase === 'build' ? 'Your recovery number turns on once Bar Cop has a few weeks to measure against, around day 30.'
        : phase === 'measure' ? 'Your first recovery numbers are landing now, up or down, and they firm up over the next few weeks.'
        : 'Your numbers are live and tracked every week.';
      here = 'You started on ' + esc(start) + ', that is day ' + dN + '. You are in the ' + this.PHASES.find(p => p.key === phase).name + ' phase. ' + next;
    }
    ...
    container.innerHTML = '<div class="screen">'
      + '<div class="sh" style="margin:2px 0 12px;">How Your Recovery Builds</div>'
      + strip + hereCard + chart + '</div>';
  },

  chartCard(weeks, start, B) {
    const title = '<div class="sh" style="margin:6px 0 10px;">Prime Cost Over Time</div>';
    if (weeks.length < 2) {
      return title + '<div class="card"><div style="font-size:12px;color:var(--t3);line-height:1.6;">Your trend fills in here as you confirm weeks in This Week. Once you have a couple of weeks, this shows your cost coming down (or drifting up) and marks exactly where your recovery number turns on.</div></div>';
    }
    const data = weeks.map(w => ({ d: w.period_end, v: w.prime_cost_pct }));
    ...
    const W = 680, H = 230, padL = 40, padR = 14, padT = 14, padB = 30;
    const plotW = W - padL - padR, plotH = H - padT - padB;
    const n = data.length;
    const x = i => padL + (n === 1 ? plotW / 2 : (i / (n - 1)) * plotW);
    const y = v => padT + (1 - (v - ymin) / (ymax - ymin || 1)) * plotH;

    const startIdx = start ? data.findIndex(p => p.d >= start) : -1;
    const measureIdx = startIdx >= 0 ? startIdx + B : -1;

    // baseline shading (your first weeks)
    let shade = '';
    if (startIdx >= 0 && measureIdx >= 0 && measureIdx < n) {
      shade = '<rect x="' + x(startIdx).toFixed(1) + '" y="' + padT + '" width="' + (x(Math.min(measureIdx, n - 1)) - x(startIdx)).toFixed(1) + '" height="' + plotH + '" fill="rgba(219,171,70,0.06)"/>';
    }
    // target dashed line
    ...
    const line = '<path d="' + path + '" fill="none" stroke="' + RT_GOLD + '" stroke-width="2" stroke-linejoin="round" stroke-linecap="round"/>';
    const dots = data.map((p, i) => '<circle cx="' + x(i).toFixed(1) + '" cy="' + y(p.v).toFixed(1) + '" r="2.4" fill="' + RT_GOLD + '"/>').join('');
    // markers
    const marker = (idx, label) => {
      if (idx < 0 || idx >= n) return '';
      const mx = x(idx).toFixed(1);
      return '<line x1="' + mx + '" y1="' + padT + '" x2="' + mx + '" y2="' + (padT + plotH) + '" stroke="' + RT_GRID + '" stroke-width="1"/>'
        + '<text x="' + mx + '" y="' + (padT - 3).toFixed(1) + '" text-anchor="middle" font-size="9" font-weight="700" fill="' + RT_TXT + '">' + label + '</text>';
    };
    const markers = marker(startIdx, 'Started') + marker(measureIdx, 'Number turns on');
    ...
    const xLabels = '<text x="' + x(0).toFixed(1) + '" y="' + (H - 10) + '" text-anchor="start" font-size="9" fill="' + RT_DIM + '">' + fmtD(data[0].d) + '</text>'
      + '<text x="' + x(n - 1).toFixed(1) + '" y="' + (H - 10) + '" text-anchor="end" font-size="9" fill="' + RT_DIM + '">' + fmtD(data[n - 1].d) + '</text>';
    // y labels
    ...
    const svg = '<svg viewBox="0 0 ' + W + ' ' + H + '" width="100%" style="display:block;">'
      + shade + targetLine + markers + line + dots + xLabels + yLabels + '</svg>';
    return title + '<div class="card" style="padding:14px 16px;">' + svg
      + '<div style="font-size:11px;color:var(--t3);line-height:1.5;margin-top:10px;">Lower is better. The shaded weeks are your baseline, your own starting point. "Number turns on" is where Bar Cop has enough to start measuring recovery against that baseline, up or down.</div></div>';
  }
};

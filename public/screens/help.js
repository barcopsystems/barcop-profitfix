  ...
  SECTIONS: [
    ...
    { t: 'Loss, Cash, and Vendors', qa: [
      { q: 'Where do loss signals show up?',
        a: 'On the Hub, under Needs Attention, as three separate rows covering the last seven days: confirmed theft adjustments, drawer shorts, and flagged spot checks. Each row opens the record it names, so a theft row lands on the Adjustment Log, a short lands on Over and Short, and a flagged check lands on Spot Check. There is no score and no separate loss page. Per-server sales-pattern theft (no-sales, voids, comps, cash mix) has its own deeper screen, Sales Integrity.' },
      { q: 'What is Sales Integrity?',
        a: 'The deep theft read. You drop a per-server sales report from your POS and Bar Cop benchmarks every server against the floor, then flags the ones whose numbers do not add up (no-sale drawer opens, void abuse, abnormal cash mix, low average check, comps, refunds) with a dollar exposure on each. It catches the register and cash games that show in the sales data; product theft like overpouring and bottle loss never reaches a sales report and is caught by pour cost, inventory variance, and spot checks instead. It is patterns to look at, not a verdict.' },
      { q: 'What happens when a variance line flags?',
        a: 'It reads Over past the standard you set for that category, and High past twice it. You see the same two words on the Variance Report and on a saved Spot Check, off the same number, so the two cannot disagree. There is no case to open and no checklist to work: the words tell you which lines are worth your time, and you chase them against the count and the register.' },

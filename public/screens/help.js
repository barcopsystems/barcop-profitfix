  ...
  SECTIONS: [
    ...
    { t: 'Audit and Fix', qa: [
      ...
      { q: 'How does Profit Fix close a gap?',
        a: 'Each gap has a fix process, an ordered set of steps that link straight to the Bar Cop screen that does the work. Tick the steps off as you go so you know where you are. Measuring does not wait on you: the fix starts on its own the first time you take a tracked action on that gap, like a count logged or a recipe costed, and Bar Cop dates it from that action. If you want to set the date yourself, Mark Implemented locks it in. From there Bar Cop measures that gap\'s weekly number against your own first weeks, and the movement is what shows on the Recovery Scoreboard.' },
    ...
    { t: 'Measuring Recovery', qa: [
      ...
      { q: 'Why does the recovery number show nothing at first?',
        a: 'Because there is nothing to measure against yet. A gap sets its baseline from your own first three weeks after the fix starts, and it needs at least one more week on top of that before a figure means anything, so about four weeks. Until then the Where You Stand card on the dashboard teaches the loop, run the audit, pick a gap, work the fix, and shows your audit opportunity, which is what is on the table and never counted as money already won.' },
      { q: 'Why do some gaps show a dollar figure and others do not?',
        a: 'Pour cost and food cost have a weekly cost percentage Bar Cop can measure, so they carry a live dollar a year at your current pace. Theft and loss and vendor control do not reduce to a clean weekly metric, so they read as a Review row you tap to work on their own screen rather than a fake dollar. Labor is part of prime cost, but its fix lives in Revenue Recovery, so it is worked there, not in Profit Fix.' },
      ...
      { q: 'What are Experiments, and how are they different from a Fix?',
        a: 'Experiments is your own before-and-after log for a change you decided to make yourself, like switching distributor or re-speccing your well pours. You name it, pick the date you made the change and the metric you are watching, and Bar Cop averages the eight weeks before against the weeks since and shows the lift. A test only starts counting a week once that whole week was lived under the change, so a test you start today reads Measuring rather than inventing a result off a week that mostly happened before it. The difference from the Fix System is who chose the work: the Fix System tracks what your audit prescribed, an experiment is your own idea. And an experiment reports a metric lift only, never recovered dollars, so it can never double-count against your measured recovery.' }
    ]},
    { t: 'The Weekly Numbers', qa: [
      ...
      { q: 'What is prime cost and why is it the headline?',
        a: 'Prime cost is COGS plus labor as a percentage of sales, the two costs you control day to day. It is the single number that tells you whether the operation is healthy, so it leads the weekly close and the dashboard. The labor in prime cost is your hourly floor labor; salaried management is a fixed cost that lands in Books, not here. It sits on the Close The Week cost strip beside the two that feed it: Bar Pour Cost, your beverage COGS against bar sales, and Food Cost, your food COGS against food sales. Each carries its own target, so you can see which side of the house moved before you go looking.' },
      { q: 'What do Check Avg, Labor %, and RPLH mean?',
        a: 'They are the three health reads that sit beside prime cost on every week you confirm. Check Avg, your check average, is your food and beverage sales divided by covers, what the average guest spent. Labor % is your total labor, hourly plus salaried, against the week\'s total sales. RPLH is revenue per labor hour, the week\'s sales divided by the hours you paid for, and it is the cleanest read on whether the schedule matched the volume because it holds up whatever your headcount is. All three appear on Confirm the Week as you close, and Week History lists them down the weeks so you read a trend instead of one night.' },
      { q: 'What does the Profit Forecast do, and what are Projected Sales and Target Upside?',
        a: 'It projects your profit forward from the weeks you have already confirmed, two ways side by side: at your current pace, and at your cost targets. Projected Sales is your recent weekly sales held forward across the window you pick, so it is a run rate off your own history, not a goal. Target Upside is the profit difference between the two columns, the money on the table if you hit your targets, and it points straight at Profit Fix. If you are already running better than your targets that tile reads Ahead of Target and shows by how much, which usually means your targets are due a look in App Settings. Every figure is a projection built from your own averages, an estimate to plan against, not a guarantee.' },

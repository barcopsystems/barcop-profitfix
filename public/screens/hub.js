...
S.Hub = {
  ...
  render(container) {
    ...
    const dot  = () => `<span style="display:inline-block;width:7px;height:7px;border-radius:50%;background:#C9A84C;margin-right:6px;flex-shrink:0;"></span>`;

    const CARD_STYLE = `background:var(--surface);border-radius:10px;padding:24px;display:flex;flex-direction:column;height:380px;overflow:hidden;min-width:0;border:1px solid rgba(201,168,76,0.4);box-shadow:0 0 24px rgba(201,168,76,0.05);`;

    const activeCard = (title, desc, scoreVal, trendVal, stats, enterScreen, mod) => `
      <div style="${CARD_STYLE}">
        <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:16px;">
          <div style="flex:1;padding-right:12px;">
            <div style="display:flex;align-items:center;margin-bottom:6px;">${dot()}<span style="font-size:10px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:var(--gold);">Active</span></div>
    ...
    // Profit stats
    const profitStats = stat('Latest Audit', lastAuditDate||'No audits yet')
    ...
    const revenueStats = stat('Latest Audit', rAuditDate ?? 'No audits yet')
    ...
    const trafficStats = stat('Latest Audit', tLatestAudit ? new Date(tLatestAudit.date).toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'}) : 'No audits yet')
      ...
      <div style="min-height:100vh;background:var(--bg);display:flex;flex-direction:column;align-items:center;padding:40px 24px 60px;overflow-y:auto;">
        ...
        <div style="width:100%;max-width:920px;background:var(--surface);border:1px solid rgba(255,255,255,0.08);border-radius:10px;padding:22px 32px;margin-bottom:24px;display:grid;grid-template-columns:repeat(4,1fr);gap:16px;">
          <div>
            ...
            <div style="font-size:26px;font-weight:800;color:var(--t1);line-height:1;">3 <span style="font-size:13px;color:var(--t3);font-weight:400;">/ 3 Systems Active</span></div>
          </div>
        ...
        <div style="width:100%;max-width:920px;display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:24px;">
          ${activeCard('Profit Recovery','Pour cost, food cost, theft, inventory variance, vendor pricing and cash control.',profitScore,profitTrend,profitStats,'dashboard','profit')}
          ${activeCard('Revenue Recovery','Menu engineering, pricing strategy, server performance, labor efficiency and events.',rAuditScore,null,revenueStats,'r-dashboard','revenue')}
          ${activeCard('Traffic Recovery','Google presence, online menu, reviews, social media, delivery platforms and digital visibility.',tAuditScore,null,trafficStats,'t-dashboard','traffic')}
        </div>
        ...
        <div style="width:100%;max-width:920px;background:var(--surface);border:1px solid rgba(201,168,76,0.13);border-radius:10px;padding:18px 24px;display:flex;align-items:flex-start;gap:14px;margin-bottom:16px;">
          ...
          <div style="font-size:13px;color:var(--t2);line-height:1.6;">Full platform visibility active. Profit, Revenue and Traffic Recovery are working together to surface operational gaps no single system can detect alone.</div>
        </div>

  ...
  SECTIONS: [
    ...
    { t: 'Operations', qa: [
      ...
      { q: 'What goes in Operating Expenses?',
        a: 'The bills that are not COGS or labor (wages and salaries both live in Labor Control, not here): rent, utilities, insurance, marketing, professional fees, software, and the rest, by category. Logging them is what lets the Month-End income statement show a real operating income instead of stopping at prime cost. Check Recurring on a bill that repeats, pick how often it hits (monthly, quarterly, or annually), and set its Due Date to when it is next actually due so Bar Cop tracks the real billing cycle; recurring bills are ongoing by default and run until you hit Stop, so fill in "Ends after" only for a bill that stops after a fixed number of months. The Date Submitted field just records when you logged it and stays on today. For a bill that moves around month to month, leave it Variable and use Repeat to copy last month forward and set the new amount. You can also switch the form to Import File and drop a CSV or Excel export. Do not enter repairs and maintenance or 3rd-party platform fees here; those are tracked in Shift Control and the weekly P&L so Books does not count them twice.' }
    ]},
    { t: 'Connections', qa: [

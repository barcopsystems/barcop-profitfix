  ...
  renderMain() {
    ...
    const catRows = this.CATEGORIES.map(c => {
      ...
      return '<tr style="' + (dim ? 'opacity:0.55;' : '') + '">'
        ...
        + '<td style="text-align:right;font-weight:700;color:var(--t1);">' + fmt$(tm) + '</td>'
        + '<td style="text-align:right;color:var(--t3);">' + fmt$(lm) + '</td>'
        + '<td style="text-align:right;color:var(--t2);">' + fmt$(ytd) + '</td>'

...
S.CashDashboard = {
  ...
  _doneKey() { return 'cash_cockpit_done_' + this.weekStart(); },   // account-synced (App.data), follows the user across devices; no per-browser suffix
  ...
  stepRow(k, done) {
    ...
    let html = '<div style="border:1px solid var(--b-edge);border-radius:var(--r);background:' + bg + ';overflow:hidden;">'
      + '<div class="c-step-head' + (isOpen ? '' : ' collapsed') + '" data-step="' + k + '" style="display:flex;align-items:center;gap:13px;padding:14px 16px;cursor:pointer;">'
      +   circle
      +   '<div style="flex:1;min-width:0;"><div style="font-size:14px;font-weight:700;color:var(--t1);">' + m.title + '</div>'

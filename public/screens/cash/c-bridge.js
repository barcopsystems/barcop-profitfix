...
/* ── Cash Recovery — Cash Bridge (profitable but broke, explained) ─────────────
   ...
   reads Cash You Kept and where every dollar went. Recurring outflows work the
   same as Operating Expenses: ongoing until you stop them (optional fixed term),
   managed in the Recurring Outflows card, edited in a popup. Reads CashEngine;
   cash_outflows is an event store (putRecord / removeRecord). */
...
S.CashBridge = {
  ...
  showHowTo() {
    App.showHelpModal('How the Cash Bridge Works', [
      ...
      { h: 'Logging The Outflows', p: ['Log the draws, loan payments, capital buys, and tax remittances here, and Bar Cop uses them in the bridge and on the Survival Forecast as scheduled cash out. Check Recurring monthly for a regular one and it carries forward every month until you stop it. Set Ends after only for a fixed-term one, like a loan with a payoff date.'] },
      { h: 'Recurring And Repeat', p: ['Your recurring series live in the Recurring Outflows card with Stop, Edit, and Delete. Stop ends a series, the months already logged stay in your bridge and forecast, it just drops off going forward. For a one-time outflow you want to log again, use Repeat to open it pre-filled.'] },

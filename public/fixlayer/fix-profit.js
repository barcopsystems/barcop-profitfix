...

  {
    ...
    process: {
      steps: [
        ...
        { kind: 'action', target: 'pour-test', targetLabel: 'Pour Test',
          title: 'Test bartender pour accuracy monthly',
          detail: 'The Variance Report tells you the shift is off. Pour Test tells you which bartender is off. Have each bartender pour their standard spec into a graduated cylinder once a month, log spec vs actual in Pour Test. The per-bartender summary flags anyone averaging more than 5 percent off spec for coaching. A 0.2 oz over-pour on a 1,000-drink-per-month bartender is real money the shift-level data misses.' },
    ...
    process: {
      steps: [
        ...
        { kind: 'action', target: 'sc-void-comp', targetLabel: 'Void and Comp Log',
          ...
          detail: 'In Shift Control, log every void and comp against the employee who rang it. Tag each comp with the category: Customer Comp or Service Recovery for loss, Staff Meal or Shift Drink for policy expense. The split keeps the Theft Risk score honest and gives the P&L a separate line for staff drinks instead of conflating them with guest comps.' },
        { kind: 'action', target: 'sc-comp-settings', targetLabel: 'Comp Authorization',
          title: 'Set a Comp Auth Threshold and enforce it',
          detail: 'In Shift Control under Setup, open Comp Authorization and set the threshold above which a manager must sign off (defaults to $25). Saving a comp over the threshold without a manager pops a soft warning the operator can override, and every override flags in Theft Risk under Unauthorized Large Comps. The bartender comping a $40 round of drinks without manager involvement is one of the most common bar-theft patterns and this is how Bar Cop catches it.' },
...

  {
    ...
    process: {
      steps: [
        ...
        { kind: 'action', target: 'yield-test', targetLabel: 'Yield Test',
          title: 'Test plate yield by cook every couple weeks',
          detail: 'Weigh three plates of a dish during service, compare against spec yield, log the cook on the line in Yield Test. Picking a dish with a recipe auto-fills the spec from recipe plate yield. The per-cook summary flags any cook averaging more than 5 percent off spec. A line cook over-portioning fries by 20 percent on a 200-cover night runs through 40 extra pounds a week, real food cost on the floor that no other screen catches.' },

...
const Onboarding={
  ...
  renderStep(){
    ...
    const barType=s.bar_type||'bar_kitchen';
    document.getElementById('ob-content').innerHTML=
      '<div class="ob-pips"><div class="ob-pip on"></div><div class="ob-pip"></div></div>'
      +'<div class="ob-heading">Welcome to Bar Cop Recovery</div>'
      +'<div class="ob-sub">Tell us about your operation to get started. You can update all of this later in Settings.</div>'
      +'<div class="form-row">'
      ...
      +'</div>'
      +'<div class="form-row">'
      +'<div class="f" style="flex:1;"><label style="margin-bottom:8px;display:block;">Operation Type</label>'
      +'<div style="display:flex;gap:10px;">'
      +'<label style="flex:1;display:flex;align-items:center;gap:8px;background:var(--input);border:1px solid var(--b1);border-radius:var(--r2);padding:10px 12px;cursor:pointer;" id="ob-type-bar-wrap">'
      +'<input type="radio" name="ob-type" id="ob-type-bar" value="bar_only" '+(barType==='bar_only'?'checked':'')+' style="accent-color:var(--gold);"/> Bar Only'
      +'</label>'
      +'<label style="flex:1;display:flex;align-items:center;gap:8px;background:var(--input);border:1px solid var(--b1);border-radius:var(--r2);padding:10px 12px;cursor:pointer;" id="ob-type-bk-wrap">'
      +'<input type="radio" name="ob-type" id="ob-type-bk" value="bar_kitchen" '+(barType==='bar_kitchen'?'checked':'')+' style="accent-color:var(--gold);"/> Bar and Kitchen'
      +'</label>'
      +'</div></div>'
      +'</div>'
      +'<div class="form-row" id="ob-rev-row">'
      ...
      +'<div class="f w-lg" id="ob-frev-wrap"><label>Annual Food Revenue (Estimate)</label><div class="fw"><span class="pre">$</span><input class="pre" type="number" id="ob-frev" value="'+(s.annual_food_revenue||'')+'" placeholder="0" /></div></div>'
      +'</div>'
    ...
    // Show/hide food revenue based on type selection
    const toggleFoodRev=()=>{
      const isBar=document.getElementById('ob-type-bar')?.checked;
      const fw=document.getElementById('ob-frev-wrap');
      if(fw) fw.style.display=isBar?'none':'';
    };
    document.getElementById('ob-type-bar')?.addEventListener('change',toggleFoodRev);
    document.getElementById('ob-type-bk')?.addEventListener('change',toggleFoodRev);
    toggleFoodRev();
    ...
    document.getElementById('ob-next1')?.addEventListener('click',()=>{
      ...
      const isBarOnly=document.getElementById('ob-type-bar')?.checked;
      App.data.settings.bar_name=name;
      App.data.settings.city_state=document.getElementById('ob-city')?.value.trim();
      App.data.settings.bar_type=isBarOnly?'bar_only':'bar_kitchen';
      App.data.settings.annual_bar_revenue=parseFloat(document.getElementById('ob-brev')?.value)||0;
      App.data.settings.annual_food_revenue=isBarOnly?0:(parseFloat(document.getElementById('ob-frev')?.value)||0);
  ...
  renderTargets(){
    ...
    const isBarOnly=App.data.settings.bar_type==='bar_only';
    const modules=App.subscription?.active_modules||[];
    const moduleNames=modules.map(m=>m==='profit'?'Profit Recovery':m==='revenue'?'Revenue Recovery':'Traffic Recovery');
    const moduleSummary=moduleNames.length
      ? moduleNames.join(' and ')
    ...
    document.getElementById('ob-content').innerHTML=
      ...
      +(isBarOnly?'':'<div class="f w-sm"><label>Food Cost %</label><div class="fw"><input class="suf" type="number" id="ob-fc" value="'+(t.food_cost_pct??32)+'" step="0.1"/><span class="suf">%</span></div></div>')
      ...
      +(isBarOnly?'':'<div class="f w-sm"><label>Food Labor %</label><div class="fw"><input class="suf" type="number" id="ob-fl" value="'+(t.food_labor_cost_pct??30)+'" step="0.1"/><span class="suf">%</span></div></div>')
      ...
      +'</div>'
      +'<div style="background:rgba(201,168,76,0.07);border:1px solid rgba(201,168,76,0.2);border-radius:var(--r);padding:12px 16px;margin-bottom:20px;font-size:13px;color:var(--t2);line-height:1.5;">'
      +'<span style="color:var(--gold);font-weight:600;">Ready to activate:</span> '+moduleSummary+'. You can add additional Recovery sections at any time from the Recovery Hub.'
      +'</div>'
      +'<div class="ob-actions">'
      +'<button class="btn btn-ghost" id="ob-back">Back</button>'
      +'<button class="btn btn-primary btn-lg" id="ob-finish">Go to Recovery Hub</button>'
      +'</div>';
